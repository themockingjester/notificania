...
const {
  NOTIFICATION_EVENT_CONFIG_TABLE_CONSTANTS,
} = require("../constants/notificationEventConfig.constant");
const {
  prepareProperObjectOutOfNotificationEventConfigDetailsForAMessage,
} = require("../utils/messageUtilities/common.message.utils");
...
const messageEnhancer = async (receivedMessage) => {
  ...
  if (serviceDetails.code == HTTP_CODES.OK && serviceDetails.success) {
    ...
    throw new Error(
      MESSAGE_LISTENER_INTERNAL_RESPONSES.UNKNOWN_SERVICE_PROVIDED
    );
  }
  if (!serviceName) {
...
const attachMessageEventBasedConfig = async (receivedMessage) => {
  ...
  let configData =
    await NotificationEventConfigService.findAllNotificationEventConfigs({
      whereClause: {
        event_id: eventId,
        record_status:
          NOTIFICATION_EVENT_CONFIG_TABLE_CONSTANTS.RECORD_STATUS.ACTIVE,
      },
    });
  let config;
  ...
  if (!config) {
    throw new Error(
      MESSAGE_LISTENER_INTERNAL_RESPONSES.NO_NOTIFICATION_EVENT_BASED_CONFIG_FOUND
    );
  }

  const refinedConfigs =
    prepareProperObjectOutOfNotificationEventConfigDetailsForAMessage(config);
  receivedMessage.eventConfig = refinedConfigs;
};
module.exports = {
  messageEnhancer,
  attachMessageEventBasedConfig,
};
