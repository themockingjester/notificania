...
class NotificationEventConfigService {
  ...
  constructor() {
    this.NotificationEventConfigRepository =
      new NotificationEventConfigRepository();
  }
  intiateDefaultRepository() {
    this.NotificationEventConfigRepository.setModel(
      exportedDIContainer.dbModels[
        DATABASE_CONSTANTS.TABLES.NOTIFICATION_EVENT_CONFIG
      ]
    );
  }
  ...
  findOneNotificationEventConfigCore = async ({ whereClause, include }) => {
    ...
    return this.NotificationEventConfigRepository.findOne({
      whereClause,
      include,
    });
  };
  ...
  findAllNotificationEventConfigCore = async ({ whereClause, include }) => {
    ...
    return this.NotificationEventConfigRepository.findAll({
      whereClause,
      include,
    });
  };
  ...
  createNotificationEventConfigCoreBulkCreate = async ({ data, options }) => {
    await this.checkForDefaultModel();

    return this.NotificationEventConfigRepository.bulkCreate({
      dataArray: data,
      options,
    });
  };
  deactivateNotificationEventConfigCore = async ({ whereClause, options }) => {
    ...
    return this.NotificationEventConfigRepository.deactivate({
      whereClause,
      options,
    });
  };
  updateNotificationEventConfigCore = async ({
    data,
    whereClause,
    options,
  }) => {
    ...
    return this.NotificationEventConfigRepository.update({
      data,
      whereClause,
      options,
    });
  ...
  addNewNotificationEventConfig = async (data) => {
    ...
    const notificationEventConfigExists =
      await this.findOneNotificationEventConfigCore({
        whereClause: {
          event_id: event_id,
          key: key,
          // record_status: 1
    ...
    if (!notificationEventConfigExists) {
      ...
      let createdConfig = await this.createNotificationEventConfigCore({
        data,
      });
      return resultObject(
        true,
        `Created notification event config successfully`,
        {
    ...
    return resultObject(
      false,
      `Another similar notification event config already exists`,
      {},
      HTTP_CODES.CONFLICT
    );
  };

  updateExistingNotificationEventConfig = async (
    data,
    notificationEventConfigId
  ) => {
    await this.checkForDefaultModel();
    ...
    const fetchedServiceConfigData =
      await this.findOneNotificationEventConfigCore({
        whereClause: {
    ...
    if (notificationEventConfigExists) {
      ...
      return resultObject(
        false,
        `Similar notification event config already exists`,
        {},
        HTTP_CODES.CONFLICT
      );
    }
    ...
    return resultObject(
      true,
      `Updated notification event config successfully`,
      {
  ...
  findAllNotificationEventConfigs = async ({ whereClause = {} }) => {
      ...
      include: [
        {
          model:
            exportedDIContainer.dbModels[
              DATABASE_CONSTANTS.TABLES.NOTIFICATION_EVENT
            ],
        },
      ],
    });
    ...
    return resultObject(
      true,
      `Fetched notification event configs`,
      {
        result: allConfigs,
      },
      HTTP_CODES.OK
  ...
  findSpecificNotificationEventConfig = async (notificationEventConfigId) => {
    ...
    return resultObject(
      true,
      `Fetched notification event config`,
      {
        result: configData,
      },
      HTTP_CODES.OK
