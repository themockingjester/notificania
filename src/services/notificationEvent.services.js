...
class NotificationEventService {
    ...
    intiateDefaultRepository() {
        this.notificationEventRepository.setModel(exportedDIContainer.dbModels[DATABASE_CONSTANTS.TABLES.NOTIFICATION_EVENT])
    }
    ...
    deactivateNotificationEventCore = async ({ whereClause, options }) => {
        ...
        return this.notificationEventRepository.deactivate({ whereClause, options })
    }
    updateNotificationEventCore = async ({ data, whereClause, options }) => {
        await this.checkForDefaultModel()

        return this.notificationEventRepository.update({ data, whereClause, options })
    }
    ...
    addNewNotificationEvent = async (data) => {
        ...
        if (!notificationEventExists) {
            ...
            return resultObject(true, `Created notification event successfully`, {
                result: createdEvent
            }, HTTP_CODES.CREATED)
        }
        return resultObject(false, `Another similar notification event already exists`, {
    ...
    updateExistingNotificationEvent = async (data, notificationEventId) => {
        ...
        if (!event_name) {
            return resultObject(false, `event name is missing`, {}, HTTP_CODES.BAD_REQUEST)
        }
        // fetching notification event
        const fetchedNotificationEventData = await this.findOneNotificationEventCore({
            whereClause: {
                id: notificationEventId
            }
        })
        if (!fetchedNotificationEventData) {
            return resultObject(false, `Notification event not found`, {}, HTTP_CODES.NOT_FOUND)
        }
        ...
        if (notificationEventExists) {
            ...
            return resultObject(false, `Similar notification event already exists`, {
        ...
        return resultObject(true, `Updated notification event successfully`, {
    ...
    findAllNotificationEvents = async () => {
        ...
        return resultObject(true, `Fetched notification events`, {
    ...
    findSpecificNotificationEvent = async (notificationEventId) => {
        ...
        return resultObject(true, `Fetched notification event`, {
